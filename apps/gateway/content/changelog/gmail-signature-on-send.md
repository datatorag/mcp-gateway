...
**Drafts stay unsigned until they are sent.** `gmail_create_draft` and `gmail_update_draft` never
add a signature. It is added once, when the draft goes out through `gmail_send_draft`, so a draft
you review and edit does not collect a second copy.
