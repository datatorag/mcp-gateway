export const EVENTS = {
  ...
  /** One run's token ceiling (SCRUM-84). Distinct from PLAYGROUND_CAP_HIT,
   * which is the per-period RUN allowance refusing a new run. Carries a
   * `reason` (SCRUM-251): `soft` when the run crossed the closing line and
