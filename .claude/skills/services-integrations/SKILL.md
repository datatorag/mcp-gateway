...
- List/template IDs (`BREVO_LIST_PRODUCT_USERS`, `BREVO_TEMPLATE_WELCOME`, `BREVO_TEMPLATE_NO_ACTIVATION`, `BREVO_TEMPLATE_FOLLOWUP_{CONNECT,PERMISSIONS,TRY_THIS}`) are hardcoded constants, not env vars — they're managed in the Brevo console. Template params are flat strings; anything list-shaped is rendered to a string in code (`listInProse`) and the template owns every other word.
...
- New server-side events in `track.ts` (names in `src/lib/analytics.ts` `EVENTS`): `playground_message_sent`, `agent_run`, `playground_cap_hit`, `playground_confirm`, `playground_feedback`, plus `tool_call` (emitted at the MCP layer only — since SCRUM-188 the agent is an in-process MCP client and the agent layer emits nothing; the event carries `surface` (now `mcp` for agent traffic too), `run_id` (null through the MCP layer), and `client_id`/`client_name` per SCRUM-189) — all never-throw, all spread `identityProps()`. Client-side (posthog-js, dashboard components): `playground_prompt_run` (a "What can I do?" card's Run button), `wizard_client_selected`, `wizard_step_completed` (setup wizard).
- **The skill funnel (SCRUM-223)** carries the slug on every hop: client-side `skill_run_cta_clicked` (public page), `skill_run_clicked` (dashboard list), `skill_run_started` (the seeded turn submitting, with `surface` and `trigger`); server-side, `user_signed_up`, `user_logged_in` and `account_connected` gain a `skill` property parsed by `skillSlugFromPath` from the SAME validated `next` the redirect redeems (never from anything else, so an event cannot claim a skill the redirect did not carry), and `agent_run` gains `skill` (validated against the catalogue in the chat route) and `trigger` (`manual | scheduled`). These are INTENT events: activation stays the first `tool_call` per person and a skill run is never substituted for it. Over MCP (SCRUM-224): `skill_searched` (`query_length`, `results`, `top_result: published | user | null`, `surface`) and `skill_applied` (`skill`, `via: prompt | tool`, `surface`, `runnable`, `trigger`), server-side from `mcp-server.ts`, plus the `tool_call` the two built-ins emit by construction. **Rule: analytics never carries user free text.** Search text is user content, the same way tool arguments are, so `skill_searched` sends the query's length and never the string; if the strings are ever needed that is a product decision with a retention policy, not an event property. `skills-catalogue.ts`'s `echoName` is the ONLY place a request value is reflected back to a requester (an unknown prompt name, tool name, server slug or skill slug): strings only, control and format characters stripped (ASCII and C1 controls, bidi overrides), capped at 64. The deliberate exception is the `echo` built-in, whose whole job is to reflect. Route any new "unknown X" message through it. User-owned skills (SCRUM-226) add `skill_created`, `skill_updated`, `skill_forked` and `skill_deleted` (`skill`, `via: tool | dashboard`, and `shadowed` on a delete), server-side through `trackSkillEvent` in `track.ts` from both the MCP built-ins and the dashboard routes, plus the client-side captures of the same names with `source: "dashboard"`; `skill_run_clicked` gains `layer`. Skill CONTENT never reaches analytics: only the slug travels. Scheduled runs (SCRUM-225) add `skill_scheduled` (`skill`, `cadence`, `hour`), `skill_schedule_paused` (`by: user | system`, `reason`), `skill_schedule_resumed`, `skill_schedule_deleted` and `skill_run_finished` (`skill`, `trigger`, `status`, `delivered`, `tool_calls`), all through `trackSkillEvent` in `track.ts`; `skill_run_started` and `agent_run` carry `trigger: "scheduled"` and `surface: "scheduler"` from the runner. **The scheduler's email is outbound copy** (`src/gateway/skills/notify.ts`, sent through `sendBrevoEmail` from `LEADS_CONFIRMATION_FROM`): one per run at most, none when the skill mailed the user itself, and `notify.test.ts` pins the house rules (no em-dashes, lowercase subject, escaped html).
- **One metering path (CRITICAL invariant, reshaped by SCRUM-188):** the agent consumes the gateway's own MCP in-process, so every tool call — agent or external — flows through the same CallTool dispatch and emits exactly one `tool_call`. The agent layer meters NOTHING: not a reduced count, no count at all. `surface` reads `mcp` for agent traffic and no longer discriminates it; `client_id`/`client_name` (SCRUM-189: `web`/`datatorag-agent` for the agent) are the provenance axis, and rows from before those fields existed have neither — a cliff, not a usage drop. The free-tier hard stop gates agent calls like every client's, deliberately.
...
`apps/gateway/src/gateway/lifecycle.ts` is the Brevo side of this pipeline: `sendWelcomeEmail()` fires on signup (skips internal emails and no-key state); `runNoActivationFollowup()` is a daily job that selects users older than 3 days with no `firstToolCallAt`, atomically claims each one (same `UPDATE ... WHERE ... IS NULL RETURNING id` pattern) **before** sending, then sends ONE of three follow-up templates and alerts `#alerts` on send failure. Which template is decided by `followupBranch()` from the user's `service_connections` rows AFTER the claim (SCRUM-140): no row → connect nudge; a row whose grant `scopeDelta` (`scope-grant.ts`, the only place that judges a grant) reports incomplete → missing-permissions email with `MISSING_SERVICES` naming what no connection covers; a complete row → try-this nudge. The branch reads connection state only to choose the template, never to decide whether to claim — one claim per user is what guarantees one email per user however their state changes between runs. Keep claim-then-send; computing the branch before the claim reads better and reopens the double-send. It checks `hasBrevoKey()` before claiming anything — claiming without being able to send would permanently burn the user's one-shot eligibility. `LIFECYCLE_LAUNCH` is a hardcoded cutoff constant so pre-existing users (who got a manual one-off campaign) don't also get the automated flow.
