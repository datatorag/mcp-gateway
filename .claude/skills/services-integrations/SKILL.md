...
- `trackSignup()` is PostHog-only (`identify` + `user_signed_up` capture). The #leads Slack post for signups lives in `apps/gateway/src/gateway/signup-alert.ts` `notifySignup(db, user)` (SCRUM-26, enriched in SCRUM-85), called from the same `isNewUser` branch in `auth.ts`: it skips internal accounts via `isInternalEmail` (the SCRUM-6 lesson: `trackSignup`'s old inline Slack line didn't). Provenance comes from the `acquisition_*` columns on the users row (read back AFTER `persistAcquisition`, so the alert reports what was actually stored): a `Source:` line (channel + utm detail or referring domain + a gclid-presence flag, never the raw gclid) and a `Landed:` line (entry PATH only, query string stripped). An absent snapshot prints `Source: unknown (no acquisition data captured)`, deliberately explicit, it's the in-channel signal that attribution capture broke. The newest `leads` row by email still feeds a secondary `Lead match:` line (`none` vs `✓ converted` + the lead's own UTM set). Never-throw, `void`-called. The `Source:` wording is `acquisitionSourceLine()` / `acquisitionSummary()`, exported from this file and shared with the customer alert and the digest, so provenance reads the same on every Slack surface.
- **The #leads post for a REAL new customer** lives in `apps/gateway/src/gateway/billing/customer-alert.ts` (SCRUM-212): `newCustomerFromEvent()` recognises exactly `customer.subscription.created` with an active/trialing status, and `notifyNewCustomer()` posts email, plan (price id resolved against the env price ids), amount, and the acquisition source line. `handleStripeEvent` awaits it after the transaction commits and only on a first delivery, so the existing event claim is the idempotence and a replay posts nothing. **Never alert on `customer.created`**: our checkout route emits it when checkout starts, before anyone pays. `payment_intent.succeeded` was rejected too: it fires on every renewal and carries neither our user nor the plan.
...
Three rules the digest now lives by (SCRUM-211/212):

- **Label an event as what it IS, never as what it sounds like.** Stripe `customer.created` is emitted by our own checkout route when checkout BEGINS, so it is "Checkouts started", and a started checkout that no subscription followed says so on its own line; a customer exists at `customer.subscription.created`. PostHog `connector_added` is a dashboard connect-button CLICK captured before OAuth (no connection, no user email), so it is "Connect clicks (dashboard)"; the connection is `account_connected`, the server-side event. Each wrong label had reported intent as an outcome, in a message that is read once and believed.
- **When two sources disagree, the digest says so.** Collectors return `{ lines, facts }` (`CollectorResult`; bare `string[]` still accepted), `reconcile()` compares the shared `FactKey`s (`connections`, `signups`) between the DB and PostHog, and `formatDigest()` appends a conspicuous "Sources disagree" block naming both numbers. Only facts the sources are MEANT to agree on go in `Facts`: a counter that undercounts by design (usage_events, best-effort insert) would mark every day and teach readers to ignore the marker.
- **Detail that makes a number actionable, not a dashboard.** The formatting is pure and tested without a database (`neonLines`, `stripeLines`, `posthogLines`): signups are named with their `acquisitionSummary()`; the paying customers are a named line, conspicuous (⚠️) at zero calls; the connection count carries its funnel denominator (`Connected: X of Y signups in the last 14 days`). Every added line has to earn its place against the digest ceasing to be read.
