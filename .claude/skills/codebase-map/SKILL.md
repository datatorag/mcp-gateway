...
- **The agent surface meters, activation does not** — this reversed on the `agent-metering` branch; the dashboard agent used to write no `usage_events` at all. Both surfaces now emit one `tool_call` event carrying `surface` (`"mcp" | "agent"`), and both write usage rows. `first_tool_call_at` is still set for `surface: "mcp"` only, because activation must keep meaning "a real MCP client called through the gateway". The per-period run allowance lives in `src/gateway/usage/period.ts` (details in the `services-integrations` skill's Playground section).
