...
- **Playground writes are user-gated** — the agent loop pauses before running any mutating tool (`isWriteTool` in `playground/tools.ts` classifies by verb tokens; reads never gate). On a write, `runPlaygroundTurn` returns `awaiting_confirmation` having executed NOTHING in that batch; the route stores the paused conversation in `playground/pending.ts` (in-memory, TTL'd, single-instance) and emits a `confirm` SSE event with a resume token. The client shows Approve/Deny; the decision POSTs `{resumeToken, decisions}` back to the SAME `/api/playground/chat` endpoint, which does NOT re-claim a cap message and calls `resumePlaygroundTurn` (runs approved writes + all reads, refuses denied writes with a declined tool_result, then continues the loop — which may pause again).
