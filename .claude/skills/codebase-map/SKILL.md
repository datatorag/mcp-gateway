...
- **The dashboard chrome is the same on every route; only the CONTENT mode differs.** `dashboard/layout.tsx` renders one icon-only 56px rail on all of `/dashboard/*` (labels live in `aria-label`/`title`, active state in `aria-current`). It also holds a `FULL_HEIGHT_ROUTES` set (`/dashboard/agent` today): a route in it gets `children` with NO padded `max-w-5xl` wrapper and a shell sized to the viewport, so the surface owns the screen and manages its own scrolling; every other route keeps the padded, document-scrolling main. Before editing: a full-height page must not add page padding or a heading of its own, and the rail deliberately is not a scroll container (`overflow-y-auto` forces `overflow-x` to auto and clips the user-menu popup, which is wider than the rail).
- **A full-height shell is NOT `h-dvh`, and this has bitten twice.** `useFitBelowTopChrome` (`src/lib/use-fit-below-top-chrome.ts`) measures the shell's own top edge and sets `height = innerHeight - top`. The reason: the marketing page loads a third-party badge widget whose fixed bar AND compensating `body { padding-top }` both deliberately survive client-side navigation, so anyone arriving from the marketing site carries an offset into the app. A `dvh` shell then keeps a full viewport height, starts lower, and pushes its bottom — and the composer anchored there — off screen by exactly the offset. A direct load has no widget and no defect, which is why route-loading checks miss it entirely: **reproduce this by navigating from `/`, never by loading `/dashboard/agent` directly.** Do not "simplify" the measurement to a constant or to `body` padding: the widget has a second, in-flow variant on iOS Safari with no padding at all. Anything inside the shell that wants full height must use `h-full` (which tracks the measured value), never `dvh` — an aside asking for `dvh` becomes taller than its own parent and gets clipped at the bottom, where the user menu lives. **That rule is scoped to full-height routes only, and the sidebar is deliberately `dvh` on the scrolling ones**: there the shell is `min-h-screen`, which is not a definite height, so `h-full` resolves against content and the rail collapses to roughly its own content height with the user menu stranded under the nav instead of pinned to the bottom. (Flexbox `align-items: stretch` does not rescue it: stretch needs the cross-size property to compute to `auto`, and `100%` does not.) The aside's two-branch height is correct; do not "simplify" it to one.
