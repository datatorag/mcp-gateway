...
- **Playground writes are user-gated** — the agent loop pauses before running any mutating tool. Write/read is decided by `classifyWrite` (`playground/tools.ts`): it trusts the MCP `readOnlyHint` annotation captured into `tools.read_only_hint` at discovery (`plugin-manager.ts`), and falls back to a verb heuristic (`isWriteTool`) only when the tool is unannotated (null). `listUserEngineTools` returns `{ tools, isWrite }` so both the fresh-turn and resume paths gate identically. On a write, `runPlaygroundTurn` returns `awaiting_confirmation` having executed NOTHING in that batch; the route stores the paused conversation + the gated writes in `playground/pending.ts` (in-memory, TTL'd, single-instance) and emits a `confirm` SSE event with a resume token. The client shows Approve/Deny; the decision POSTs `{resumeToken, decisions}` back to the SAME `/api/playground/chat` endpoint, which does NOT re-claim a cap message and calls `resumePlaygroundTurn` (runs approved writes + all reads, refuses denied writes with a declined tool_result, then continues the loop — which may pause again). Plugins should set `annotations.readOnlyHint` on their tools so the gate uses declared intent instead of the name heuristic.
