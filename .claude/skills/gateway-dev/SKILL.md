...
4. **Do not add a per-plan agent-run allowance.** Pro sells CALL volume only; the run cap is the same constant on every tier until a token ceiling is measured (cost asymmetry — runs burn our model budget). Tests in `plans.test.ts` and the chat-route suite go red if plan-shaped data reaches the run claim.
