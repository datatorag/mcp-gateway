...
1. **The user's effective plan is `users.plan`** (denormalized, kept in sync ONLY by the webhook handlers — nothing else may write it). The `subscriptions` table is the Stripe-side record; `users.plan` is the hot-path read.
