...
1. **New built-in tool** (like `echo` / `list_connected_accounts`, no plugin process involved): add an entry to the `BUILT_IN_TOOLS` registry in that file — definition plus handler in one object. The registry drives ListTools, CallTool dispatch, AND `tool_call` emission with `builtin: true` (metered:false, no billing sink, no activation claim), so an entry inherits correct tracking by construction; do NOT hand-roll an inline `if (name === ...)` branch, which is exactly how the two originals shipped emitting nothing. `mcp-server.builtins.test.ts` iterates the registry, so a new entry is test-covered automatically.
...
4. **New OAuth-connect provider** for a service: mirror the Google/Atlassian pair in `apps/gateway/src/gateway/auth.ts` — a scopes const (like `GWS_SCOPES`/`ATLASSIAN_SCOPES`) plus a `/auth/<provider>/connect` route + callback, gated on the `dtrmcp_session` cookie. If the provider's consent screen lets users untick individual scopes (Google does), also teach `src/gateway/scope-grant.ts` about it — the required-scope list, any granted-string aliases, and the tool-prefix→scope map — or partial grants will be recorded as clean connections (the SCRUM-136 failure). A new untickable scope also needs a row in that module's `API_SCOPES` with an explicit `iconKey` that `ServiceIcon` knows (`scope-grant.icons.test.ts` pins the join), so the per-service panel (SCRUM-106, `src/app/dashboard/connections/grant-panel.tsx`) picks it up with its brand mark and no UI list has to change. User-facing strings for that panel live in `grant-copy.ts` and are pinned by `grant-copy.test.ts` (no em-dashes, no scope URLs, no service counts).
...
### Extend billing (Stage 1 is live in code)

Stage 1 flat subscription exists end to end: Checkout (`/api/billing/checkout`), Billing Portal (`/api/billing/portal`), signature-verified idempotent webhooks (`/api/stripe/webhook` → `billing/webhook-handlers.ts`), and free-tier hard-stop enforcement (`billing/enforce.ts` `checkCallAllowance`, called in `mcp-server.ts` CallTool before dispatch). When touching any of it:

1. **The user's effective plan is `users.plan`** (denormalized, kept in sync ONLY by the webhook handlers — nothing else may write it). The `subscriptions` table is the Stripe-side record; `users.plan` is the hot-path read. Plan is derived from subscription STATUS alone — keep it that way, because a portal cancellation on the current Stripe API version arrives with `cancel_at` set and `cancel_at_period_end` still `false` (verified against a real portal cancel in test mode); any logic keyed on the boolean would miss real cancellations, and the `subscriptions.cancelAtPeriodEnd` column already under-records them for the same reason.
2. **Webhook handlers must stay idempotent.** Every event id is claimed in `stripe_events` (INSERT … ON CONFLICT DO NOTHING) inside the same transaction as the handler's writes; a failure rolls back the claim so Stripe's retry processes fresh. New event types go through the same `applyEvent` switch and inherit the claim. The acceptance test for any change here is REPLAYING the event and watching the second delivery change nothing (`webhook-handlers.test.ts`, real Postgres).
3. **Enforcement gates BEFORE dispatch, counting happens after.** `checkCallAllowance` refuses the next call; `countToolCall` (unguarded, deliberate) records the one that ran. Don't merge them.
4. **The agent-run allowance is PER PLAN and lives in `planLimits()`** (SCRUM-84 — this reversed the earlier plan-independent rule once the per-run token ceiling was measured and ruled): free 25 runs/month, pro 100, everything else falls to free. Do NOT reintroduce a free-floating per-plan constant next to `FREE_MONTHLY_AGENT_RUNS` — a parallel const outside the plan table is the shape that shipped Pro capped at the free allowance unnoticed. The cost bound is allowance x `RUN_TOKEN_CEILING` (150k tokens per run, all plans, enforced by `mastra/run-token-budget.ts` refusing the run's NEXT model call at a step boundary, surfaced to the user via the chat route's failure tap — never truncating an in-flight call). Tests in `plans.test.ts`, the chat-route suite, and `run-token-budget.test.ts` pin all of it, mutation-proven.
5. Metered overage / usage ledger is Stage 2 and deliberately unbuilt: a COUNTER enforces a cap; a LEDGER bills (dedup, replay, audit). Don't wire `usage_events` to Stripe as a side effect of something else.
6. For usage-threshold notifications (80/100/150%), reuse the dedupe pattern already modeled by the `alert_sends` table (composite PK on `(userId, periodStart, thresholdPct)`) even though nothing currently writes to it.
7. Stripe API access goes through `getStripe()` / `ensureStripeCustomer()` in `apps/gateway/src/lib/stripe.ts` — it's the only client that throws (not no-ops) on a missing key, so guard calls accordingly in dev. Price ids come from env (`STRIPE_PRO_MONTHLY_PRICE_ID` / `STRIPE_PRO_YEARLY_PRICE_ID`), never hardcoded.
